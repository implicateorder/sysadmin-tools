...
"\n$tstamp:\t$host console is in use...please investigate manually\n";
        ...
        [
            ...
            sub {
                ...
                print WOF
"\n$tstamp:\t$host - Attempting to drop down into ILOM/SC prompt\n";
                $exp->send("#.\n")
                  or print WOF
"\n$tstamp:\t$host - Unable to drop down into ILOM/SC prompt\n";
                sleep($timeout);
        ...
        [
            ...
            sub {
                ...
                $exp->send("$user\n")
                  or print WOF
                  "\n$tstamp:\t$host: Unable to send $user to login: $! \n";
              }
        ],
        [
            qr'ssword:\s*$',
...
sub testSP {
    ...
    my $conslog = "/var/tmp/console_log_$date.log";
    open( WOF, ">> $logfile" ) or die "Unable to write to/open logfile: $! \n";
    print WOF "$tstamp:\tRunning testSP $ssh -l $user $host \n";

    my $sshcmd = Net::SSH::Expect->new(
        host     => $host,
        user     => $user,
        password => $dexpasswd,
        raw_pty  => 1,
    ...
    $sshcmd->run_ssh()
      or ( print WOF "Unable to ssh to $host as $user:$! \n" and die );
    #( $sshcmd->read_all(2) =~ /->\$\>\#\]\s*\z/ )
    #  or ( print WOF "Can't find a prompt for $host: $!\n" && die );
    ( $sshcmd->exec("exit\n")
          && print WOF "Successfully connected to $host as $user: $! \n" )
      or
      ( print WOF "Can't execute exit cmd on $host ssh session:$! \n" && die );
...
elsif ( $type =~ m/SC|iLo|ILOM|ALOM/i ) {
